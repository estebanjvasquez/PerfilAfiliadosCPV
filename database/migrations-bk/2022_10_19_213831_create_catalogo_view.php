...
class CreateCatalogoView extends Migration
{
    ...
    public function up()
    {
        ...
        empresas.id as id, empresas.name, sectors.name as Sector, empresas.street, CONCAT(cities.city_name, ', ', countries.country_name) as CIUDAD, 
        empresas.ano_fund as 'fundacion', IF(empresas.property_id LIKE '1', 'Privado', 'Publico') AS Propiedad, 
        IF(empresas.origin_id LIKE '1', 'Nacional', 'Internacional') AS Origen,

        GROUP_CONCAT(DISTINCT contacts.name, ' (', contacts.position, ', ', contacts.phone, ', ', contacts.email, ') ' ORDER BY contacts.name SEPARATOR '; \n') as CONTACTOS
    from 
        empresas
